            ...
            {showDeveloperDebug && (
                <div className="card card-sm" style={{
                    ...
                    <div style={{ marginTop: '12px', borderTop: '1px solid rgba(0,0,0,0.1)', paddingTop: '12px' }}>
                        ...
                        <div style={{ fontSize: 11, color: '#8395A7', marginBottom: 8 }}>
                            {Object.entries(joinedChallengeIds).map(([uid, ids]) => {
                                const u = users.find(user => user.id === uid);
                                return ids.length > 0 ? (
                                    <div key={uid}>{u?.name || uid.slice(0, 6)}: {ids.length}件</div>
                                ) : null;
                            })}
                            {Object.values(joinedChallengeIds).every(ids => ids.length === 0) && '参加なし'}
                        </div>
                        <button
                            onClick={() => {
