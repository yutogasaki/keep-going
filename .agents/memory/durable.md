...
- ふわふわタイプは通常タイプに季節タイプを足す方式。通常 type `0-9/14-16`、2026-04 は type `10/11`、2026-05 は type `12/13` を50%の月別バイアス候補にし、月別バイアスを外した通常抽選では通常タイプを優先する。開発者モードは `FUWAFUWA_TYPES` から選べる状態を保つ。
...
- CI の verify 正本は `.github/workflows/verify.yml` で、`lint -> npm run typecheck -> test -> build` を `pull_request` と `main` push で回す。
