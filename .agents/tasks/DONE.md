...
- 既存の `soundVolume` は `ガイダンスと効果音` 用として整理し、効果音側のゲインを見直して TTS だけ大きく感じやすかったバランスを調整した
- `npx tsc --noEmit`、`npm test`、`npm run build`、desktop/mobile の設定画面 visual QA を通した
...
- さらに `新規7日 / 非アクティブ7日+ / 休止候補7日+` に基準を寄せ、一覧の複数選択から `休止` と `復活` をまとめて実行できるようにした
