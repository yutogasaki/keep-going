...
- developer dashboard の運用基準は `新規14日保護`、`非アクティブ14日+`、`休止候補 = 30日超の長期未利用 + 14日超の未使用整理候補` を正本にする。未開始放置とリセマラ疑いは同じ `未使用整理候補` にまとめ、整理候補ユーザーの一括削除は member のみ削除して account は残す。
